...
from codegen.files import file_system, get_line_endings, lib_dir
...
def test_that_code_is_up_to_date():
    ...
    mtimes = {}
    for fname in file_system.filenames_to_change:
    ...
    file_system.reset()
    assert file_system.filenames_written == set()

    # Collect original version of the files
    file_cache = {}
    for fname in file_system.filenames_to_change:
        file_cache[fname] = file_system.read(fname)

    # Perform the codegen
    main()

    # Confirm that all files that are allowed to change, are actually written
    assert file_system.filenames_written == set(file_system.filenames_to_change)
    ...
    for fname in file_system.filenames_to_change:
        content1 = file_cache[fname]
        content2 = file_system.read(fname)
