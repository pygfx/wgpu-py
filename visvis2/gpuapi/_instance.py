...
class Instance(GPUObject):
    ...
    def __init__(self, validation_layers=None):
        ...
        # Create handle
        self._handle = self._create_handle(validation_layers)
    ...
    def _create_handle(self, validation_layers):
        ...
        # Get the extensions that GLFW needs
        import glfw  # todo: looks like I need a surface before I can instantiate the instance!
        extensions = [str(x) for x in glfw.get_required_instance_extensions()]

        # Add extension that we need if we use validation layers
        if validation_layers:
            extensions.append(vk.VK_EXT_DEBUG_REPORT_EXTENSION_NAME)

        # todo: for extensions that we want but don't *need*, we may need to check
        available_extensions = [x.extensionName for x in vk.vkEnumerateInstanceExtensionProperties(None)]
