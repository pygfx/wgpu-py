...
# %% Generate code for flags, enums and structs

print("\n##### Generate API for flags, enums and structs")
...
for name, d in ip.flags.items():
    pylines.append(f"{name} = Flags('{name}',")
...
for name, d in ip.enums.items():
    pylines.append(f"{name} = Enum('{name}',")
    for key, val in d.items():
        pylines.append(f"    {key}={val!r},")
    pylines.append(")\n")
...
# Write
with open(os.path.join(lib_dir, "_constants.py"), "wb") as f:
    f.write("\n".join(pylines).encode())
print("Written to _constants.py")
